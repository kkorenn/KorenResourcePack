...
if [ -z "${GAME:-}" ]; then
  if [ "$(uname)" = "Linux" ]; then
    GAME="$HOME/.steam/steam/steamapps/common/A Dance of Fire and Ice"
  else
    GAME="$HOME/Library/Application Support/Steam/steamapps/common/A Dance of Fire and Ice"
  fi
fi
...
#   FORCE_BUNDLE=1 ./scripts/koren_build.sh
...
# Convert any OTFs in the selected Unity font folder to TTF. Unity loads TTF
# more reliably than CFF-based OTF.
OTF_FILES=("$UNITY_PROJECT"/Assets/Font/*.otf)
if [ -e "${OTF_FILES[0]}" ]; then
  python3 tools/otf2ttf.py "${OTF_FILES[@]}"
fi
  ...
  if [ -z "$UNITY_BIN" ]; then
    ...
    if [ "$NEED_BUILD" = "1" ]; then
      if [ -f "$UNITY_PROJECT/Temp/UnityLockfile" ] && lsof "$UNITY_PROJECT/Temp/UnityLockfile" >/dev/null 2>&1; then
        echo "[Bundle] Unity Editor already has $UNITY_PROJECT open."
...
DIST="$SRC/dist"
BUILD_DLL="$SRC/build/bin/$BUILD_SUBDIR/$OUTPUT_NAME.dll"
DEST="$GAME/Mods/KorenResourcePack"
if [ -f "$DEST/$OUTPUT_NAME.dll" ]; then
  shasum -a 256 "$BUILD_DLL" "$DEST/$OUTPUT_NAME.dll"
else
  shasum -a 256 "$BUILD_DLL"
fi
