...
# Run once before ./koren_build.sh.
set -e

SRC="$(cd "$(dirname "$0")" && pwd)"
cd "$SRC"

UNITY_VERSION="$(awk '/m_EditorVersion:/ {print $2}' KorenResourcePack-Unity/ProjectSettings/ProjectVersion.txt 2>/dev/null || echo "6000.3.10f1")"
...
if command -v python3 >/dev/null 2>&1; then
  ...
  warn "python3 — only needed if Fonts/ contains .otf files (auto-converts to .ttf). Install: brew install python  /  apt install python3"
fi
...
echo
if [ "$MISSING" = "1" ]; then
  echo "Some required deps missing. Install them, then re-run ./setup.sh."
  exit 1
fi
