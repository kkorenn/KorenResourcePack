    ...
    internal static partial class JudgementRestriction
    {
        ...
        private static void TriggerFail(string reason)
        {
            try
            {
                ...
                if (m != null)
                {
                    // FailAction(bool overload, bool multipress, string failMessage, bool hitbox).
                    // overload=true routes through scrCountdown.ShowOverload() and the
                    // "Too Many Keys" screen, which blocks normal respawn. Pass false so the
                    // game runs the standard miss-fail flow (state Fail2 -> retry works).
