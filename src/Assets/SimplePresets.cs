    ...
    internal static class SimplePresets
    {
        ...
        private static int ComputeSettingsHash(int style)
        {
            unchecked
            {
                ...
                AddFloatHash(ref hash, Main.SettingsRef.KeyViewerSimpleFootOffsetX);
                AddFloatHash(ref hash, Main.SettingsRef.KeyViewerSimpleFootOffsetY);
                AddFloatHash(ref hash, Main.SettingsRef.KeyViewerSimpleRainWidth);
                AddFloatHash(ref hash, Main.SettingsRef.KeyViewerSimpleRain2Width);
                AddFloatHash(ref hash, Main.SettingsRef.KeyViewerSimpleRainOffsetY);
                AddFloatHash(ref hash, Main.SettingsRef.KeyViewerSimpleRain2OffsetY);
                ...
                AddFloatHash(ref hash, Main.SettingsRef.SKvGhostRainR); AddFloatHash(ref hash, Main.SettingsRef.SKvGhostRainG);
                AddFloatHash(ref hash, Main.SettingsRef.SKvGhostRainB); AddFloatHash(ref hash, Main.SettingsRef.SKvGhostRainA);
                return hash;
            }
        }
                                      ...
                                      float dx, float dy, float w, float h)
        {
            ...
            float noteWidth = SimpleRainWidthForGroup(colorGroup);
            if (noteWidth > 0f) p["noteWidth"] = noteWidth;
            p["noteOffsetY"] = SimpleRainOffsetYForGroup(colorGroup);
            p["ghostNoteColor"] = ColRgba(Main.SettingsRef.SKvGhostRainR, Main.SettingsRef.SKvGhostRainG, Main.SettingsRef.SKvGhostRainB, Main.SettingsRef.SKvGhostRainA);
            p["ghostNoteOpacity"] = Mathf.Clamp01(Main.SettingsRef.SKvGhostRainA) * 100f;
        ...
        private static float SimpleRainWidthForGroup(int colorGroup)
        {
            return colorGroup == 1
                ? Main.SettingsRef.KeyViewerSimpleRainWidth
                : Main.SettingsRef.KeyViewerSimpleRain2Width;
        }

        private static float SimpleRainOffsetYForGroup(int colorGroup)
        {
            return colorGroup == 1
                ? Main.SettingsRef.KeyViewerSimpleRainOffsetY
                : Main.SettingsRef.KeyViewerSimpleRain2OffsetY;
        }
