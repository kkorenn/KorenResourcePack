...
rem Run once before koren_build (or `dotnet build -p:Install=true`).
...
if exist "KorenResourcePack-Unity\ProjectSettings\ProjectVersion.txt" (
  for /f "tokens=2" %%v in ('findstr /b "m_EditorVersion:" KorenResourcePack-Unity\ProjectSettings\ProjectVersion.txt') do set UNITY_VERSION=%%v
)
...
if %ERRORLEVEL%==0 (
  ...
  echo   [warn]    python - only needed if Fonts\ has .otf files. Install: winget install Python.Python.3
)
...
echo.
if "%MISSING%"=="1" (
  echo Some required deps missing. Install them, then re-run setup.bat.
  popd
  exit /b 1
)
